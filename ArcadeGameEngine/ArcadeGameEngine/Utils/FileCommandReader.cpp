...
void FileCommandReader::AddCommand(const Command& command)
{
	mCommands.push_back(command);
}

bool FileCommandReader::LoadFile(const std::string& filePath)
{
	std::ifstream inFile;
...
Color FileCommandReader::ReadColor(const ParseFuncParams& params)
{
...
Vec2D FileCommandReader::ReadSize(const ParseFuncParams& params)
{
	size_t nextSpacePos = params.line.find_first_of(" ", params.dilimitPos + 1);
...
int FileCommandReader::ReadInt(const ParseFuncParams& params)
{
	std::string intStr = params.line.substr(params.dilimitPos + 1);
	return std::stoi(intStr);
}

std::string FileCommandReader::ReadString(const ParseFuncParams& params)
{
	return params.line.substr(params.dilimitPos + 1);
}

char FileCommandReader::ReadChar(const ParseFuncParams& params)
{
